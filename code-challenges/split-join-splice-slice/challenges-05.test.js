...
const howMuchPencil = (str) => {
  let result = [];
//  [essentially, run the array through a for loop or forEach and have it slice off a letter at a time using str.slice(0) and return shows each resulting slice.  Would I need two variable strings so that it changes them out each time it goes through?]
  return result;
};
...
const wordsToCharList = (arr) => {
  // I would use split("") method because the input string appears that it would not have any separators and have it return the input as separator letters/characters.
  arr.split();
  return arr;
};
...
const listFoods = (recipe) => {
  let result = [];
  // I would run the first slice through to separate each line item by the space or I could use the Array.indexOf() method and remove all of the numbers on each of the line items and/or remove the first two elements since it appears that the number and measurement increment are the first two items of each line.  That would leave me the rest of the line, which should be the ingredient.  I could even do search for a specific word and cut it out.  Ex. gallons
arr.slice(0,1);
  return result;
};
...
const splitFoods = (recipe) => {
  let result = [];
  // const words = str.split(' ');  console.log(words[2]); - something like this where I have it look at the words in the line and remove the first two based on a space separator.  It would need to go through each line.  Would that be a forEach or a for loop?
  return result;
};
...
const stepActions = (recipe) => {
  let result = [];
  // Similar to the one above, I would go through each line and take off the first word, which would be identifiable with the space separator and put that into a new array.  Again, I would need it to go through each line and do this.  Would this be a forEach or a for loop?
  return result;
};
...
const removeEvenValues = (arr) => {
  // I would have a math function as part of it to find out if % 2 to determine if the integer is an even or odd number.  If it figures out it is an even number, then I would have it push into an array.  I would have it go through each line (a for or forEach loop??) and have it determine and push the even integer.  The return would be the array that is created by the even integers.  for (let i = 0; i < array.length; i++){     if (i % 2 ===0){       arr.push(array[i]);
};
